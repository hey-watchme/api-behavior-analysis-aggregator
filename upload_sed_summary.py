...
FastAPI経由でクラウドストレージ（EC2）にアップロードする。
"""
...
class SEDSummaryUploader:
    ...
    def find_all_summary_files(self) -> List[Tuple[str, str, Path]]:
        ...
        # パターン: /Users/kaya.matsumoto/data/data_accounts/{user_id}/{YYYY-MM-DD}/sed-summary/result.json
    ...
    def find_summary_file(self, user_id: str, date: str) -> Optional[Path]:
        """
        特定のユーザー・日付のサマリーファイルを取得
        """
        file_path = self.base_dir / user_id / date / "sed-summary" / "result.json"
