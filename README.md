...
3. **クラウド統合**
   - EC2データ取得: `https://api.hey-watch.me/status`
   - クラウドアップロード: `https://api.hey-watch.me/upload/analysis/sed-summary`
   - SSL証明書検証無効化対応
...
- aiohttp並列HTTP処理（最大30並列接続）
- 音響イベント再帰的ラベル抽出
- 時間ブロック別データ構造化
- ローカルファイル保存（JSON形式）

**upload_sed_summary.py:273行** - アップロード処理
- FormDataでのファイルアップロード
- 全ファイル/特定ファイル両対応
- 成功・失敗詳細レポート機能
- SSL証明書問題の自動回避
...
**データ処理フロー**
1. HTTP並列取得 → 2. JSON解析 → 3. ラベル抽出 → 4. 集計処理 → 5. ファイル保存 → 6. クラウドアップロード
...
**📥 データ収集**: EC2上のSEDファイル（最大48個の30分スロット）を非同期並列取得  
**📊 データ集計**: 音響イベントラベルの自動抽出・集計処理  
**📤 データアップロード**: クラウドストレージへ自動アップロード  
...
**🌐 ネットワーク:**
- `https://api.hey-watch.me/status` へのHTTPS接続
